...
function ProjectHeader({
  ...
  const user = useSelector((state: RootState): RootState["user"] => state.user);
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event: MouseEvent) {
      if (
        ...
        !projectPanelRef.current.contains(event.target as Node)
      ) {
      ...
      <div className="mt-1 flex flex-wrap items-end gap-5">
        ...
        {visibleProjects.map((p) => (
          <button
            ...
            className={`project-header__project ${
              p.id === currentProject ? "project-header__project--selected" : ""
            }`}
        ...
        <div ref={projectPanelRef} className="relative project-panel-wrapper">
          <button
            ...
            onClick={() => setIsProjectModalOpen((prev) => !prev)}
          >
            <FolderOpen size={20} />
            All Projects
          </button>
